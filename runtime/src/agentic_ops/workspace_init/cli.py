...
def execute_workspace_init(
    ...
    if interactive and not sys.stdin.isatty():
        raise _blocked(
            ...
            "零参数工作空间初始化需要终端输入",
            "请在终端运行 agentic-cli workspace init，或使用完整非交互参数",
        )
...
def execute_workspace_preflight(
    ...
    if workspace.config_path is None:
        raise _blocked(
            ...
            "业务项目工作空间尚未初始化",
            "请先运行 agentic-cli workspace init",
        )
    install_root = (
        Path(install_root_value).expanduser().resolve()
        if install_root_value
