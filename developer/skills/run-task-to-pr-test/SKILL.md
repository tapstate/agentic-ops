...
description: Coordinate one explicitly authorized real Jira task through a real business repository to a real GitHub pull request and, for development_change_v2, through bounded CI observation and remediation, recording every step in the local task-run audit protocol and producing a truthful ready, blocked, or failed result package. Use in a developer business-project workspace after task intake and solution classification; Runtime prepares the integration manifest from confirmed task facts. Never merge, move Jira to Done, release, tag, push a protected branch, or modify AgenticOps source.
metadata:
  workplane: developer
---
...
10. `development_change_v1` 保持原行为，在 PR 审查节点停止。`development_change_v2` 不进入内置代码审查，而是在 manifest CI 配置与当前 Project Profile 精确一致时进入 `ci_validation`：
    - `ci_policy` 固定为 `detect_from_github_pr`。执行 `ao-work task-run probe-ci --manifest <...>` 后，Runtime 绑定 PR Head/Base，从 GitHub 读取 Base 提交的 Workflow 树和 Blob；Base 不含 Workflow 文件时返回 `not_required` 并结束，适用的已配置 Workflow 会由 PR/Head 推送触发时返回 `required`。读取失败、树截断、名称缺失/重复或触发语义不一致时转人工；不得根据项目名、聊天声明或本地文件猜测。
    - 只有 `required` 才为每个 Head 建立 5 分钟启动截止时间。观察到任一匹配 Workflow Run 或检查后，建立独立的 10 分钟完成截止时间。恢复运行不能重置任一截止时间，之后按 Profile 的 15 秒间隔观察。
    - 尚未观察到 CI 执行时只继续观察到 5 分钟启动截止时间；已执行但未结束时只继续观察到 10 分钟完成截止时间。任一超时都必须人工介入。缺失、重复或未知必需检查均不能通过；只有所有必需检查对最终 Head 唯一且结论均为 `SUCCESS` 才是 `passed`；`NEUTRAL`、`SKIPPED` 不得视为通过。
    - `failed` 时执行 `fetch-ci-artifact`，Runtime 只接受当前 Head 唯一失败 Workflow Run 和唯一匹配的未过期 Artifact，并在版本化大小、数量、深度及路径安全上限内落盘；随后执行 `parse-ci-report`，只能消费结构化脱敏报告，不能读取或执行原始报告脚本。
    - 解析后必须先分类，不能直接修复。只有明确判定为业务代码缺陷时，才以 Runtime 返回的 `failure_event_id` 记录唯一 `failure` 事件，并固定使用 `code=ci_code_defect`、`retry_safe=true`；修复还必须位于已确认 `scope.included`、不命中 `scope.excluded`、不改变测试预期/Workflow/规则且无需多方取舍。修复后重新执行全部本地验证，使用受控 commit/push，执行 `probe-git` 回读新 Head，再以 `record-ci-remediation` 把失败事件、修复提交、新 Head 和原授权绑定；之后观察新 Head。
...
只有 Runtime 可信 Jira/Git/PR/CI probe、真实 Jira Comment 与 Worklog 专用写后回读、真实 `git_commit`、全部 Runtime 验证、五项禁止动作和复盘都闭合，才可选择 `ready_for_pr_review`。v2 还必须存在绑定最终 PR Head/Base 的 CI Attempt：GitHub 判定 `required` 时全部必需检查严格为 `SUCCESS`，判定 `not_required` 时必须有 Base Workflow 证据且不得包含伪造的检查、运行或修复事实；该既有结果状态表示 AIAgent 已完成开发闭环，不表示 PR 已合并或 Jira 已 Done。到达终态前阻塞或失败时仍要记录已经发生的外部动作与回读、禁止动作审计、完整分类复盘和下一步，再生成 `blocked` 或 `failed` 结果；不得补造缺失事实或伪装通过。任一禁止动作 `observed=true` 都是事故，只能定性为 `failed` 并保留越权证据，不能输出 `ready_for_pr_review` 或普通 `blocked`。
