...
10. `development_change_v1` 保持原行为，在 PR 审查节点停止。`development_change_v2` 不进入内置代码审查，而是在 manifest CI 配置与当前 Project Profile 精确一致时进入 `ci_validation`：
    ...
    - 尚未观察到 CI 执行时只继续观察到 5 分钟启动截止时间；已执行但未结束时只继续观察到 10 分钟完成截止时间。任一超时不再只抛出错误：Runtime 返回绑定当前 PR、必需检查、Workflow Run 与截止时间的 `analyze_ci_timeout`。AI 必须主动分析这些事实，向用户说明 CI 是否未触发、排队、仍运行或已有异常，并请求“继续等待 / 转人工排查 / 其它项目认可处置”的决策；不得静默无限轮询、重置截止时间或重跑 Workflow。缺失、重复或未知必需检查均不能通过；只有所有必需检查对最终 Head 唯一且结论均为 `SUCCESS` 才是 `passed`；`NEUTRAL`、`SKIPPED` 不得视为通过。
