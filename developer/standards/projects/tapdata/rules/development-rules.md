...
新功能开发（问题版本为 `tapdata` 主仓 `develop`）时，产品域任务工作树必须先运行 `tap_align_branches.py plan`，以脚本结果作为各仓库实际目标分支；`profile.yaml` 的 `branches.dev_branches` 只作为不经过产品域任务工作树链路时的静态项目映射。
...
- `tapdata` 为 `develop` 时：除 `tapdata-license` 切 `main` 外，其它联动仓（含 `tapdata-common-lib`）均切 `develop`。
...
`profile.yaml` `branches.dev_branches` 保留下列静态项目映射（确定性，不靠 AI 猜测）。产品域任务工作树创建仍以前述 `tap_align_branches.py plan` 为事实源：

| 仓库 | 新功能开发分支 |
| --- | --- |
| `tapdata/tapdata` | `develop` |
| `tapdata/tapdata-cloud` | `develop` |
| `tapdata/tapdata-common-lib` | `develop` |
