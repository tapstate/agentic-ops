...
def _verify_superseded(operation, name, step):
    ...
    if not isinstance(history, list) or not isinstance(current, dict) or not name.startswith(("resource:", "source-reset:", "workspace-source-reset:", "external:", "clear-active:", "archive-publish:")):
    ...
    elif name.startswith("workspace-source-reset:"):
        if (revision["plan"].get("schema_version") != 4
                or name != "workspace-source-reset:" + str(original_revision) + ":" + original
