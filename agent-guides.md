...
本文是安装后 AIAgent 的全局启动入口。研发负责人在项目 AI 工作空间启动 AIAgent 后，应发送：
...
5. 读取当前工作空间 `.agentic-ops/profiles/<workspace>.yaml`。
6. 执行 `agentic-cli preflight`。预检失败时停止接管任务，并把缺失配置、权限或路径问题说明给研发负责人。
...
- 不依赖研发负责人个人 Obsidian wiki、个人长期记忆或上一段聊天上下文。
