...
用户要求判断 AO 工作项、修复或能力是否已经实现时，必须先确定要评估的代码事实，不能默认把当前工作树当作最新实现。作出“已实现”“部分实现”或“未实现”结论前必须：

1. 只读刷新官方远端引用，并记录当前 `HEAD`、刷新后的 `origin/main`、`origin/develop`、当前分支及其领先、落后或分叉状态。远端无法刷新时必须明确标记“当前主线证据不足”，不得把缓存引用或本地文件描述成最新远端事实。
2. 根据问题选择并声明唯一评估基线及其 SHA：已经稳定交付或询问当前可用能力时使用刷新后的 `origin/main`；尚在开发或等待合入时使用 Jira、任务分支或 GitHub PR 明确绑定的当前 Head。不得仅因本地分支名为 `main` 或 `develop` 就把它当作对应远端基线。
3. 当前工作树未包含评估基线、落后或已分叉时，不得只引用工作树路径和行号得出能力缺口。必须通过固定 Git 对象（例如 `git show <sha>:<path>`）、隔离 worktree 或隔离快照读取并验证目标基线；文件引用同时说明对应 ref 或 SHA。
4. 分别核对 Jira 需求边界、Git 提交包含关系、目标基线代码、测试和必要发布事实。设计文档、提交说明、测试名称或 Jira 状态任一项都不能单独证明能力已经实现。

结论必须区分以下状态，不得相互替代：

- `主线能力缺口`：目标基线本身缺少需求能力或验收证据。
- `本地检出未同步`：目标基线已经实现，但当前工作树尚未包含对应提交。
- `安装或部署未更新`：源头基线已经实现，但目标安装、工作空间或运行环境尚未更新到该基线。
- `证据不足`：远端、PR、提交、测试或运行环境事实无法完成必要核对。

阶段进度和剩余工作仍以 Jira 为事实源；本节只约束如何选择和说明 Git 实现证据，不把实现状态复制到长期文档。
...
原子步骤成功不是会话终点。AIAgent 必须在当前用户目标、授权、范围和风险边界内持续消费 Runtime 返回的结构化 `next_step`，直到到达真实人工节点、流程终态或明确阻断。接管、分析、补卡、实现、验证、证据回写、提交、推送或 PR 等任一原子操作的 `ok=true`、`status=completed`、内部 action、digest、plan id 或“操作成功”都不能单独作为最终答复。

维护 Runtime、release.sh、hotfix.sh、development-workflow.sh 的产品 JSON 都必须输出 `step-result/v2`，并在各自工作面独立通过抽象 `Step` 子类构造唯一 `next_step`；不得恢复或输出 `agentic_next_action`。共享目录只允许保存纯 JSON Schema 和示例向量，禁止共享 Runtime 代码、角色策略或副作用能力。
