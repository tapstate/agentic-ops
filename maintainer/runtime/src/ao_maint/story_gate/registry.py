...
from ao_maint.story_gate.model import (
    ACCEPTANCE_CHECKS,
    STORY_CATEGORIES,
    StoryContract,
    StoryRegistry,
...
REGISTRY_PATH = "maintainer/standards/stories/project-quality.yaml"
...
def load_story_registry(root: Path) -> StoryRegistry:
    ...
    if not isinstance(categories, list) or set(categories) != STORY_CATEGORIES:
        raise ValueError("故事注册表必须且只能声明 maintainer、developer")
...
def _parse_story(root: Path, raw: dict[str, Any], index: int) -> StoryContract:
    ...
    expected_prefix = "PM-" if category == "maintainer" else "DE-"
