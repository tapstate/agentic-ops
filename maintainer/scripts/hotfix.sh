...
# shellcheck source=maintainer/scripts/lib/development-workflow.sh
...
case "$command_name" in
  ...
  prepare)
    ...
    printf '{"ok":true,"operation":"hotfix_prepare","jira_id":"%s","branch":"%s","version":"%s","head":"%s","verified_at":"%s","tag_action":"reuse_only","protection_mode":"%s","delivery":"python_source_and_developer_assets","agentic_next_action":"review_release_scope"}\n' \
      "$HOTFIX_JIRA_ID" "$HOTFIX_BRANCH" "$hotfix_version" "$hotfix_head" \
      "$RELEASE_VERIFIED_AT" "$protection_mode"
    ;;
  publish)
    release_require_command git || exit 1
    release_require_command "${AGENTIC_OPS_GH_BIN:-gh}" || exit 1
    ...
    release_verify_story_gate "$repo_root" origin/main "$hotfix_head" || exit 1
    release_run_full_verification "$repo_root" "$hotfix_head" || exit 1
