...
推送、创建拉取请求、重新提交修复和合并都属于人工门禁，只有在研发工程师明确授权的范围内才能执行。研发工程师确认版本化设计或修复计划时，可以形成工作项级连续执行授权，覆盖同一 Jira 工作项内的实现、验证、提交、任务分支推送、必要 Jira 回写以及创建或更新拉取请求；这些动作不再逐项暂停，但必须统一停在拉取请求审查。日常变更只允许推送 `develop` 或符合规则的任务分支，不得直接推送 `main`；正式发布和 Hotfix 必须使用第 8 节脚本，在完整验证后取得最终确认。
