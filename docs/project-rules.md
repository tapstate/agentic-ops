...
Hotfix 只能从干净且与 `origin/develop` 精确同步的本地 `develop` 执行。Jira key 为必填 Git 审计标识，脚本不读取、不修改、不评论 Jira，也不调用 `gh`。脚本不创建修复分支、PR 或 Tag，不执行完整发布验证，不等待额外人工确认；调用命令本身就是本次快速修复授权。`origin/main` 必须是 `origin/develop` 的祖先，脚本以两者为父提交生成带 Jira key 的 Merge commit，原子推送到远端 `main` 和 `develop`，再同步本地 `develop` 并回读。分叉、未同步、脏工作区或原子推送失败时关闭，禁止 rebase、cherry-pick、强推或部分更新。
