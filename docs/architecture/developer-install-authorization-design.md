...
developer 安装代表一名研发工程师，统一保存该研发工程师的 `agent_id`、Git/GitHub 执行身份、Jira email 和 Jira API token。业务项目工作空间只绑定 Project Profile、源码位置和当前安装身份引用，不再保存、修改或迁移授权凭据。
...
`ao-work auth` 交互式配置或更新安装级身份与 Jira 凭据；重复执行即为单独重新授权。非交互模式可提供 `--agent-id`、`--jira-email`、`--git-name`、`--git-email`、`--github-login`、`--token-stdin` 和 `--non-interactive`。token 不提供字符串参数。
...
新初始化结果固定为 `agent.json` schema v4：
...
schema v3 和工作空间 `.agentic-ops/.env` 不再是运行时授权来源：

- schema v3 在读取工作空间 `.env` 或访问 Jira 前返回升级阻断。
...
- schema v4 仍以 `install_identity_ref` 防止误用其它研发工程师安装；身份漂移只能显式重新初始化，不能由普通 preflight 重绑。
...
1. `ao-work auth` 可交互或非交互写入安装目录 `user/identity.yaml` 与 `user/.env`，权限为 `0600`，输出不含 token。
...
5. `workspace init` 缺安装身份或凭据时在外部访问前阻断；成功结果固定 schema v4，工作空间不产生或修改 `.env`。
6. schema v3 不读取旧 `.env`，只输出重新授权和显式重新初始化动作。
...
- schema v3 不自动迁移 token，旧工作空间需要人工重新输入一次，但避免跨作用域静默复制凭据。
- 安装已经完成但授权失败时形成可恢复的部分状态；输出必须明确区分，不得宣称研发工程师已就绪。
- 修改安装身份会让既有 schema v4 工作空间触发 `install_identity_drift`；必须逐个显式重新初始化，不能批量静默重绑。
