...
- 任务可以闭合一个或多个实际变更仓库。shared manifest v1 继续保持单仓合同；多仓任务由一次任务级确认拆成多个仓库交付单元，每个单元生成一份 manifest 和独立协议状态，不把多个仓库塞入一份单仓 manifest。
...
- `change_repository`：兼容单仓实际变更仓库 slug；或 `change_repositories`：多仓实际变更仓库 slug 数组；
...
- 根因与实现方案；
- 全部实际变更仓库、固定工作树、来源/任务/目标分支及逐仓当前 HEAD；
- 包含与排除范围；
...
`authorize` 只在研发工程师明确确认当前 `confirmation_package` 后调用。用户不复制 draft id、plan SHA 或 manifest SHA；Runtime 从当前任务运行读取唯一待确认 draft，重新验证绑定事实，记录一次设计决策与连续执行授权，再按仓库原子输出。兼容单仓路径为：

- `inputs/agentic-ops/<issue>/<run>/approved-plan.md`；
- `inputs/agentic-ops/<issue>/<run>/task-to-pr.manifest.json`。

多仓路径为 `inputs/agentic-ops/<issue>/<run>/repositories/<可读仓库前缀>-<slug摘要>/` 下的逐仓批准计划和 manifest；摘要避免合法仓库名规范化后的目录碰撞。文件名和目录由 Runtime 决定，调用者不能越界或覆盖现有文件。单仓成功输出继续兼容 `manifest_path`；多仓输出 `deliveries[*].manifest_path`。Skill 随后逐仓自动调用现役 `task-run open --manifest <manifest_path>`，每仓协议状态保存到独立目录，不再请求第二次确认。
...
| repository | L1 执行计划选择的每个仓库在 `confirmed_repository_branch_map` 中对应的 `worktree_status=prepared` 工作树及 Git 回读；每份 manifest 只绑定其中一个仓库 |
...
3. 生成包含全部交付单元的批准计划，并为每个仓库副本计算同一 `approved_plan_sha256`；
4. 逐仓填入同一授权引用、确认声明和确认时间；
5. 使用 `task-run open` 同一 `manifest_digest(...)` 逐仓计算 `confirmed_manifest_sha256`；
6. 逐仓调用同一 `validate_manifest(...)`、验证命令校验和工作空间/任务绑定预校验；
7. 全部通过后才发布批准计划和最终 manifests，任一仓库失败都不返回部分成功授权。
...
1. TAPSTATE-87 等价 fixture 从 L1 solution 选定仓库、包含多个 prepared 工作树的 product 领域集合和 Profile 生成完整确认包；默认仓库与实际 connector 仓库不同仍绑定后者。
...
6. 多个领域 prepared 工作树不阻断协议；Runtime 只处理执行计划明确选择的仓库。多仓执行计划必须逐仓声明 scope 与验证，任一选中仓库未准备、领域外或 HEAD 漂移时使完整确认包失效。
...
2. task-run 仓库绑定改用 AO-97 执行计划对应的 prepared 工作树，不再绑定 `agent.json` 默认仓库；同领域多工作树用于分析，多仓实际修改在同一完整设计中逐仓展示并一次审查。
...
- shared manifest v1 仍不在一份文件内表达多仓提交/PR；任务级编排通过逐仓生成 manifest 覆盖真实多仓交付，因此无需升级 shared schema。每仓仍必须独立验证、提交、推送和创建 PR。
- AO 建卡允许跨出业务 Project 创建专用反馈卡，但权限只限版本化 `Agentic 缺陷` 创建协议，不能扩大为 developer 接管或维护 AO 任务。

若实施需要修改 shared manifest schema、把多个仓库塞入单一 manifest、放宽验证/摘要/外部动作门禁、改变保护分支规则、读取真实业务凭证或新增其它 AO 操作，视为范围扩大并重新进入设计审查或风险决策。
