...
Agent 是执行主体；AgenticOps 只提供标准、规则门禁和少量确定性状态。Project/Policy 和合同定义成果要求，可替换脚本辅助执行，Workflow 按真实成果和有效授权验收并更新状态，不以辅助脚本的成功回执代替结果判定。确定性入口复用版本化标准和判定，Gate 不理解平台字段。旧版实现固定在 `v0.7`，能力只在解决当前问题时按新分层重建。
