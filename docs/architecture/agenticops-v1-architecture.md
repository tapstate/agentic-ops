...
同版本生成→任务闭环→purge→生成先独立验收。升级是第二层：原版清理成功并确认干净边界→切换产品→目标版生成。当前 epoch 4、最低 updater 2；本次不发过渡版本，旧安装须原版本解绑后重新安装。新 updater 不解析另一版本任务，不以 repair 在线收编旧状态。多个已登记工位均须预检；错误或无法核验时保持原产品版本。
