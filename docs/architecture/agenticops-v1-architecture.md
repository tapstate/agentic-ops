...
本文描述当前已实现的产品内核。工作空间已选择向单任务研发工位演进，目标合同独立见[单任务工位设计合同](single-task-station.md)，方向以[项目目标](../strategy/project-goals.md)为准。在新状态代际实现并发布前，本文的多任务目录、Source Pool/worktree 与恢复规则仍是现役行为；不得用目标合同中的接口直接操作旧工作空间。
