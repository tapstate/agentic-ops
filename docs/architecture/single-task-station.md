...
删除中断后恢复同一操作。版本 3–5 的 delete_root 缺失按原创建/删除意图与父身份核验；版本 6 按原确认、保全覆盖、父身份及实际缺失核验，不要求 Agent 补造脚本删除意图。clear_children_keep_root 必须证明原根身份未变且为空。范围变化通过同一 operation 追加计划修订和确认；正式档案不可覆盖，新增源码必须明确 export/discard 或另行安全保存，不能假称被旧档案覆盖。
...
版本 6 的目录快照、结果接力及准入语义不兼容旧版读写，station_state_epoch 从 18 提升至 19。升级与回退前要求原版本结束任务并 purge，目标版本不解析或迁移旧 epoch 的活动任务。保留的旧计划实现仅保持各自合同，不把旧计划改成版本 6；它们不是新清理的依赖。原版本缺陷需在原版本维护恢复，不以新版本直接清理旧现场。
