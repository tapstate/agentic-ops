...
首次接管从可信 origin 准备缺失仓库；已有仓库先核验路径、origin 和洁净度，再 fetch 明确引用并检出冻结 SHA，不改工位之外的主工作树。若冻结引用是 branch，则检出只由 AgenticOps 管理、名称含原引用与冻结 SHA 的本地基线分支；tag 和指定 commit 保持 detached。独立仓库保留任务分支和提交，退出时回到已确认的项目开发分支，空闲刷新和下一次接管分别核验新版本。缺失或冲突不覆盖原仓库；只重试该操作能够证明归属的创建或同步步骤。
...
export 必须写到本次清理范围以外的受控位置，实际内容、源码快照及回读均核验；discard 必须额外确认精确仓库、路径和内容指纹。无任何有效动作时不得恢复或删除源码。敏感正文不进入档案，不通过改写 patch 假称保存完整；超过大小上限或不支持的冲突索引、submodule、特殊对象停止并明确处置；源码链接只读取和保存链接文本，不跟随目标。旧档案必须逐成果摘要覆盖，不因整体档案有效就推定保存了新增成果。归档保存脱敏总结、结构化证据和 Project 指定 runtime/logs、runtime/reports 文本；停止期间新增日志先追加不可变回执。归档保存上述脱敏材料，不保存原始敏感日志或依赖缓存。
...
接管时按 Project 每仓 dev_branch 解析已下载开发引用，保存完整 SHA、引用来源、观察时间及配方版本，独立于任务冻结基线。重置检出各仓 dev_branch，并仅快进至已确认且本地存在的开发 SHA；开发分支含独有提交、分叉或被其它工作树检出时停止，不覆盖成果，不执行 rebase、reset、stash 或网络 fetch。计划列出受管 agenticops/baseline 引用及原 SHA，只回收名称与 SHA 相符、提交仍可由开发目标或保留成果到达且未被其它工作树使用的引用。任务分支默认保留。任务 Head 通过 refs/agenticops/archive/<run>/<repository-digest> 保留，该引用不自动删除。空闲刷新及下次接管分别核验新版本。

空闲工位可执行 `station source-update [--repo owner/repo]`，持工位锁逐仓核验并快进已准备仓库的开发分支；有当前任务或未完成操作时拒绝，不刷新 Wiki，不克隆缺失仓库，不新增任务状态机。批量失败保留逐仓结果，再次执行收敛。

开发分支归位和受管基线回收改变清理计划及引用恢复语义，station_state_epoch 提升至 25。epoch24 及更早工位必须由原版本归档退出并 purge，再切换版本重建。外部终态回执复用档案脱敏规则保存副本及原资源摘要，不改活动事实或放宽敏感检查。
...
清理复用同一个 operation、计划摘要和回执，按 admission（准入与范围）、archive（停写与保全）、source（源码复位）、disposition（分支与 PR）、resources（登记资源）、release（最终释放）推进。正常路径连续执行；每个阶段检查实际成果，通过才进入下一阶段，不逐阶段索取人工确认。已有清理决定覆盖当前范围时复用；未完成任务拒绝继续时不启动破坏性动作。

只读盘点与可执行计划分开。完整计划因异常不能生成时，仍按仓库返回 source_inventory，列出暂存、工作区、未跟踪和 ignored 对象，以及多个特殊对象或读取错误；部分观测不授权删除。执行阶段报错返回 phase、具体 problems、原操作和已完成回执，Agent 在原范围补齐后恢复同一请求。模型不逐文件编排正常路径。

源码阶段按仓库保留成功进度，异常汇总后停止阶段出口。归档必须覆盖全部待破坏成果；外部资源已停止即可执行源码阶段，不要求提前删除。登记资源阶段才要求外部资源终态。源码阶段通过后发现新增内容，重新验收失败且保留新内容，不沿旧确认重删。未提交文件、源码链接（含断链）的实际内容或链接文本可重建，链接目标永不作为保全或删除对象；祖先路径、仓库根和受管根的链接仍拒绝。

分支及 PR 默认保留。明确删除/关闭的对象在准入阶段登记精确 before、保护观察及 decision_ref，进入原计划摘要；本地任务分支须 before.location=local，且匹配当前任务登记分支和原 SHA，不能是目标分支或 main/master/develop，也不能被任意工作树检出。Workflow 在源码复位后按旧 SHA 原子删除本地引用并回读，成果保留引用不删除。远端及 PR 由平台原生能力执行，station_disposition 在同一 run 的 cleanup_disposition 阶段接受原范围内 intent/unknown/readback；缺失或未知结果停止本阶段，不能自动改为保留。已释放任务仍可通过原档案追加独立处置，其他活动任务不允许旧任务处置。

阶段回执以 cleanup-stage:<plan-revision>:<plan-digest>:<stage> 保存在现有 steps 中，不建立第二套状态机。恢复核验实际成果；已验收阶段不重复执行。计划修订使受影响阶段重新验收，原完成回执保留，未完成意图按已有 superseded 机制闭合。最终验收再次核对所有必要终态后才清除授权、CAS 解绑；解绑后回执中断按原操作恢复。

阶段式清理与 CI 集成测试合并版本保留计划 schema 6，曾将 station_state_epoch 提升至 24；开发分支归位后的当前边界见上节。合并前清理与 CI 集成测试候选分别使用 epoch 23、语义不同，合并版本不将任一候选工位视为兼容。旧工位必须在原版本结束任务并 purge 后再切换，不在线读取或迁移旧任务；0921 等旧现场的恢复不由本版自动完成。
...
内置执行器按仓库批量恢复 Git 索引和工作区，使用 literal pathspec 有界批次避免通配和参数长度歧义；未跟踪及 ignored 内容只删除已保全且确认的路径，随后检出基线并回收受管目录。`--prepare-only` 完成范围绑定和正式保全后返回 `awaiting_cleanup_result`，Agent 可按原范围完成动作；默认执行器异常也可由 Agent 接管。`--verify-result` 不运行执行器，直接核验实际成果后通过正式入口撤销授权及解绑。无需指定工具成功退出、补齐脚本中间回执或重跑已经完成的动作；中断意图以 `observed_result` 如实闭合，不冒称原脚本执行成功。
...
源码中的普通文件默认保全，ignored 不代表可直接丢弃；大文件或敏感材料要求安全导出或明确丢弃。空目录也纳入范围，内置脚本只删除已确认的空目录。挂载、嵌套 Git、冲突索引和不支持的 submodule 状态先明确处置；普通源码链接按 Git 对象与链接文本保全和复位，不递归强删。报告与其它源码旁文件采用相同保全规则，不靠构建工具发现报告。
