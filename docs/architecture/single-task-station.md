...
接管时按 Project 每仓 dev_branch 解析已下载开发引用，保存完整 SHA、引用来源、观察时间及配方版本，独立于任务冻结基线。重置只使用已确认且本地存在的 SHA：所有基线均以 detached HEAD 检出精确 SHA，不移动命名开发分支或任务分支，不执行 rebase、reset、stash 或网络 fetch。任务 Head 通过计划中明确展示的 refs/agenticops/archive/<run>/<repository-digest> 保留；此引用不自动删除。下次接管重新刷新、核验目标开发基线。
...
此改动保留计划 schema 6 的字段结构，但改变检出、链接成果、分支处置和操作恢复语义，station_state_epoch 提升至 24。合并前清理与 CI 集成测试候选分别使用 epoch 23、语义不同，合并版本不将任一候选工位视为兼容。旧工位必须在原版本结束任务并 purge 后再切换，不在线读取或迁移旧任务；0921 等旧现场的恢复不由本版自动完成。
