...
按[维护指引](maintenance-guide.md#7-维护-agent-协作与模型适配)核验模型依据，保持 Skill 本体平台无关。入口只保留触发范围、结果、必要约束和条件性资料导航，不复制模型手册或所有工具步骤。

审查“必须确认”“立即停止”“必须测试”等措辞时，明确区分缺失输入、可自主选择、真实授权边界和失败处置。已提供且仍有效的输入或授权不重复索取；可选字段缺失不阻塞；测试授权不自动成为修复授权，但用户已明确覆盖修复范围时不重复请求。遇到授权失效、外部写结果未知或门禁拒绝，仍停止依赖操作并给出可执行接力。

仅在实际需要时加载引用资料。委派指令说明具体子任务、数量、隔离范围、停止点与证据回读；不默认给所有维护任务增加子代理。异步等待和中途补充要求使用 Agent 已提供的能力，不在通用 Skill 写 API 参数或新增状态调度。
...
本地固定测试只证明代码和接线合同。至少还要分别从源码产品根目录、一个真实业务工位和一个无关仓库启动 Agent，验证可见 Skill 集合符合上表；该结果不能替代 Jira、PR/CI 或发布验收。
