...
- `worktree_domains` 的成员不得重叠，`baseline_repository` 必须属于该领域；`problem_version_repository` 必须属于 `repositories.list`，缺失时兼容使用 `baseline_repository`。TapData TM、FE、connector 使用 `tapdata/tapdata` 作为问题版本来源，connector 归入 `product` 候选领域。
...
- 使用 `tapdata/tapdata` 产品域的 Profile（包括仅切换 Jira 项目的 TapState）使用版本化 `tap_align_branches.py plan --no-fetch --remote-only` 计算领域内逐仓分支。Runtime 在创建任何工作树前解析全部远端提交，已有工作树也必须与同一批提交一致。
