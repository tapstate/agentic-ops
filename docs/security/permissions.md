...
原则：**权限系统里做不到的事，就不需要规则去"禁止"。** 使用者工位的流程检查在 Workflow 执行；原生工具调用依靠平台权限、凭证最小化和服务端保护。
...
新工位不生成通用 Agent Hook；已有工位迁移须明确接受外部调用不再受 AgenticOps 逐次检查。不得用关闭平台沙箱或扩大凭证权限消除原生确认。源码仓库 Git Hook 只负责 AgenticOps 自身提交与发布治理。
