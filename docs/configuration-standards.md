...
本文定义 AgenticOps 配置项的分类、落点、读取方式和变更审查要求。后续任何配置相关调整都必须先按本文判断，不符合本文的方案必须先形成审查分析，再由研发工程师决策。
...
交互式初始化收到研发工程师确认的本机配置后，应先持久化非密钥配置和 secret，再执行可能耗时或失败的源码下载。workspace overlay、`agent.json` 和 `AGENTS.md` 管理块只在源码准备完成后写入，避免外部操作失败造成“配置丢失但工作空间看似已初始化”的半状态。
...
如果实现方案绕过统一配置模块、让单个功能直接解析 YAML / `.env`、新增第二个 token 名称、把 secret 写入 YAML，或让文档与 CLI 行为不一致，必须先停止实现，输出审查分析和推荐方案，等待研发工程师决策。
