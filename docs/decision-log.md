...
| D-053 | developer Git/SSH/gh 授权按全局复用或安装隔离显式选择 | 本决策扩展 D-052，不改写其安装级单入口语义。`ao-work auth` 必须区分 commit author/committer、Git SSH 远端认证和 GitHub CLI 账户，并显式选择 `global` 或 `installation`；旧身份缺模式时失败关闭。公司网络环境继续使用 SSH：`installation` 在安装 `user/` 下维护独立 Ed25519 密钥、严格受信主机资产和隔离 `GH_CONFIG_DIR`，固定通过 `ssh.github.com:443`、`IdentitiesOnly` 与禁用 Agent 回退执行；首次 GitHub 授权使用官方交互设备登录，Bootstrap 初始下载/clone 仍使用调用者已有账户，授权后 managed clone 的后续更新使用安装 SSH。`global` 只回读复用机器现有授权且绝不改写。已有状态在写入前脱敏检查：受管差异须绑定当前 `change_digest` 精确确认，私钥、不同安装 `gh` 账户、自定义 `core.sshCommand`、非受管路径和宽松权限不得静默覆盖。模式、执行身份与安装公钥指纹纳入 `install_identity_ref`；项目验证子进程不得继承安装 SSH/`gh` 凭证，`gh` 登录不得表述为 SSH push actor 证明。 |
