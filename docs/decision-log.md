...
| D-048 | 中央克隆池 + Git Worktree 源码管理（AO-92 修订） | 池成员是保留主工作树的普通完整 clone，只作为固定远端 SHA 的分析源。接管不再按默认仓库预建工作树；分析产出非权威 `proposed_repository_branch_map`，研发工程师修正并确认 `confirmed_repository_branch_map` 后，Runtime 才按实际需要在 `<pool_root>/.worktree/<jira>/<repo>/<normalized-from-branch>` 创建子工作树。TapData TM、FE、connector 的问题版本来源统一为 `tapdata/tapdata`，对齐脚本建议不覆盖用户确认。任务状态分开记录确认范围与 Git/验证/远端/PR 回读形成的实际变更仓库；Jira 完成总结逐仓汇报，完成态回读后执行非强制安全清理，源码池成员永不删除。身份继续使用 per-worktree config + 运行时 env，安装级身份/凭证边界不变。 |
