...
| D-048 | 中央克隆池 + Git Worktree 源码管理（多仓库分支推导） | 业务源码从「按工作空间独立克隆」改为「中央克隆池 + 任务级子工作树」。池根 `source_pool_root` 写入安装目录 `~/.agentic-ops/user/config.yaml`（必配），未配置 `workspace init` 阻断 `source_pool_root_invalid`、不做兼容回退；池成员为 `<owner>/<repo>` 普通完整克隆并保留主 checkout，浅克隆认领自动流式 `git fetch --unshallow`。任务接管时按 `<pool_root>/.worktree/<jira_id>/<问题版本>/<repo>` 用 `git worktree add --detach` 挂出任务级子工作树集（问题版本含 `/` 时替换为 `-`），身份用 per-worktree config + 运行时 env 双保险。多仓库通过 `repositories.list` + `worktree_domains` 领域配置 + Jira 描述「目标仓库/问题版本」section 解析；TapData 领域缺失或目标仓库未映射时失败关闭，不做全量挂载。创建任何工作树前先刷新领域内全部池成员；产品域必须调用项目分支对齐脚本的只读 remote-only JSON `plan` 计算实际仓库分支，不得回退本地 refs，其它领域使用各自基线规则。`问题版本` 与目标仓库实际 `target_branch` 分开记录，再统一解析远端提交；已有工作树也必须匹配该批提交。检测到旧布局 Jira 任务根时停止并由人工迁移或清理。阶段二把身份/凭证上移安装目录并修订 D-046。 |
