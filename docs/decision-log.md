...
| D-031 | 源头仓库采用正式分支和脚本发布流程 | GitHub 默认分支为 `main`，日常开发使用 `develop`；正常发布禁止直提直推 `main`，只允许通过 PR 的 Merge commit 合入，并使用 `maintainer/scripts/release.sh` 完成验证、确认、合并事实校验和审计。Hotfix 的后续例外由 D-055 定义。 |
| D-032 | 长期采用 GitHub Free 私有仓库软门禁 | `tapstate/agentic-ops` 保持私有仓库且不升级 GitHub 套餐。正常发布在硬门禁可用时由 Ruleset 和 Auto-merge 强制；当前 GitHub Free 私有仓库显式使用本地 Hooks、发布脚本、固定发布分支、人工 Merge commit、合并事实校验、二次完整验证和审计组成的软门禁。软门禁不能在 GitHub 服务端阻止其它 clone、`--no-verify` 或网页操作直接修改 `main`，项目明确接受该剩余风险，不再保留套餐升级待办。Hotfix 不使用该门禁模式，按 D-055 执行。 |
...
| D-047 | 源码克隆不设超时，用流式进度、停滞提示与阶段日志保障可观测性 | 大仓库 + 慢网络下 `workspace init` 的 `git clone` 无限等待，`--progress` 输出经 stderr 实时转发给用户自行判断快慢；stderr 持续无输出超过 30s 输出停滞提示（只提示、不终止进程）；`Ctrl+C` 中断由初始化回滚清理，不残留污染。该决策修订 2026-08-17 曾选择暂不做的“阶段进度日志（方案 A）”，并取代固定 120s 克隆超时。preflight 的 `ls-remote` 等秒级 Git 检查保留 60s 超时；对明确的 DNS、连接超时、连接关闭/重置和 SSH 握手中断，Runtime 仅在只读预检内按 1s、2s 退避自动重试，最多 3 次。权限、仓库身份和未知失败不重试；clone 仍不使用该重试策略。失败 JSON 保留 `stderr_tail` 并增加实际 `remote_access_attempts`。 |
| D-048 | 中央克隆池 + Git Worktree 源码管理（AO-92、AO-97 修订） | 池成员是保留主工作树的普通完整 clone，只作为固定远端 SHA 的分析源。接管不预建工作树；Runtime 从任务建议 `product`、`assistant` 或 `taptest` 领域，研发工程师只确认领域，建议不准时可指定领域重新分析。确认后 Runtime 固化自动推导的 `confirmed_repository_branch_map`，并在 `<pool_root>/.worktree/<jira>/<repo>/<normalized-from-branch>` 创建完整领域子工作树集合。product 的问题版本来源统一为 `tapdata/tapdata`，逐仓分支由对齐脚本推导。L1 执行计划可选择一个或多个实际变更仓库；任务只确认一次完整设计与连续授权，Runtime 按仓库生成 shared schema v1 manifest、独立提交、验证、推送、PR 和 task-run 状态。同领域其它 prepared 工作树只用于分析，不因未修改而阻断。任务状态分开记录领域范围与 Git/验证/远端/PR 回读形成的实际变更仓库；Jira 完成总结逐仓汇报，完成态回读后执行非强制安全清理，源码池成员永不删除。身份继续使用 per-worktree config + 运行时 env，安装级身份/凭证边界不变。 |
| D-049 | Jira 任务状态流转采用受控通用 transition 能力，maintainer 面先行（AO-23） | 两个工作面各提供 `jira transition` 命令（plan/apply/readback 同构，复用各自 WritePlan 协议），实施顺序为 maintainer（`ao-maint`）先、developer（`ao-work`）后，包独立实现不互导但 D-037 匹配规则对齐。D-037 落地为共享匹配器：稳定 ID 优先、无 ID 时名称兜底需唯一且 from/to 匹配、候选重复/目标不符/不可用/回读不一致一律阻断、禁止模糊匹配；developer 接管服务使用同一匹配器且行为兼容。完成态默认禁止 AIAgent 置 `完成/Done`（无合入权），profile 可显式声明例外；maintainer 面由维护者操作不受限。快速适配路径为设计准绳：状态/transition/字段映射全部配置化（developer profile、maintainer 独立工作流映射文件），缺省可用渐进补充（未配 id 走名称兜底、maintainer 无映射可用 `--transition-id` 安全退化），映射失配时 details 输出「当前状态 + Jira 可用 transitions 完整列表 + 已配置条目 + 未映射清单」作为可直接照抄的适配材料，表单属性按名探测 + 降级记录。主链路自动推进状态作为后续增强由版本化 Skill 编排 `jira transition` 实现，本期不实现；不新增 Jira 状态。 |
...
| D-051 | developer 接管采用 `Assignee + Status + 受管 Comment + 本地状态` | 业务 Jira 的统一入口 `ao-work takeover [<KEY>]` 处理新接管、接纳存量和恢复，成功类型固定为 `new_takeover`、`accept_existing_task`、`resume_takeover`，`blocked` 只作为失败结果；后两类在人可见输出和 Comment 中明文提示“不是新接管”。用户明确说“接管 <KEY>”即授权事实明确的常规接管，Runtime 在 run 确定后绑定稳定内部授权摘要，不再增加普通准入摘要、通用方案摘要或内部标识确认；接管后连续完成信息分析和方案分级，只在设计审查、代码审查或风险决策暂停。初始接管只验证项目、Assignee、Status/transition、Agent 身份和恢复事实，`task_class`、`process_id`、目标仓库与验证方式在接管后补齐并在实现前校验。developer 不创建、映射、探测或读写 Agentic Jira Custom Field；Comment 记录可见轨迹，Status 表达团队阶段，Assignee 表达负责人，本地状态负责运行恢复和幂等。task-to-PR 以受管接管 Comment 验证正式接管。当前不把 Comment 声称为并发锁，出现真实并发需求后专题设计。该决策不修改 AO maintainer 工作面的专用字段模型。 |
| D-052 | developer 授权固定为安装级 `ao-work auth` 单入口 | developer 安装可以在 Bootstrap 中可选接收身份与凭证参数，但安装脚本只转交给 Python Runtime；未传参数时，有终端进入授权引导，无终端完成安装并明确提示后续运行 `ao-work auth`。`ao-work auth` 同时配置或更新安装级 `agent_id`、Jira email/token、Git author/committer 与 GitHub login，`--show` 只返回脱敏状态；删除 `ao-work install identity|auth` 和 `ao-work auth jira` 多级入口。`workspace init` 只继承当前安装授权并生成 schema v4 `install_identity_ref`，不再接收、保存或读取工作空间身份与凭证。schema v3 在读取旧工作空间凭证前失败关闭，需先重新授权再由指导员明确重新初始化；Runtime 不自动复制或删除旧凭证。 |
| D-054 | 原子步骤成功不是会话终点 | 本决策同时约束 maintainer 与 developer 工作面。AIAgent 必须在当前用户目标、授权、范围和风险边界内持续消费 Runtime 返回的 `agentic_next_action`，直到到达真实人工节点、流程终态或明确阻断；接管、分析、补卡、实现、验证、证据回写、提交、推送和 PR 等任一原子操作的 `ok=true`、`status=completed`、内部 action、digest、plan id 或“操作成功”均不能单独作为最终答复。信息不足时停在信息补充门禁并展示缺失项与补卡建议；设计完整时停在设计审查并展示完整设计；事实、所有权、授权或风险冲突时停在风险决策并展示事实、选项、推荐与影响；恢复时必须恢复真实 pending gate 及其正文。允许停止的其它情形仅包括完成、交接、取消等流程终态，需人工输入、权限或外部事实变化才能解除的结构化阻断，以及用户显式限定的单步、只读或诊断请求；仅由 AgenticOps 自身能力缺口造成的阻断按 D-056 进入连续性人工决策。该原则不得用于跨越人工门禁、扩大授权、绕过能力目录或重试上限，也不得在输入不变时循环执行。 |
| D-055 | Hotfix 采用 Jira key 绑定的 develop 直合 main | `maintainer/scripts/hotfix.sh` 只接受一个位置参数 `<KEY>`：不创建分支、PR、Tag，不调用 Jira/`gh`，不执行完整发布验证或额外人工门禁。显式命令调用即为授权；脚本从任意干净分支启动，自行刷新、切换或创建 `develop`、快进落后的本地分支，并把本地领先提交纳入候选；随后自动合并固定的 `origin/main` 与 `develop`，生成带 Jira key 的 Merge commit，并用 atomic push 同步远端 `main/develop` 后回读。真实分叉或内容冲突时失败关闭。本例外不放宽正常发布、普通直推、强推或历史改写规则。 |
| D-056 | AgenticOps 自身故障不得直接终止业务开发 | AgenticOps 缺陷、能力缺口、内部状态异常或配套工具故障不能单独成为业务开发终止理由。安全自检、有限重试和只读对账无法恢复，且继续开发与 AO 控制功能冲突时，必须进入 `agenticops_continuity_decision`：展示故障证据、已确认业务事实、等待修复/项目原生有限继续/人工接手选项、精确授权与验证边界、损失的自动化审计能力、剩余风险和推荐。人工授权只覆盖限定任务、仓库、分支、动作、验证和时效；不得伪造 Runtime 成功，也不自动放行合并、发布、Tag、保护分支写入、强推、历史改写和结果不明确的外部写入重试。AgenticOps 维护修复与业务开发分轨推进。 |
