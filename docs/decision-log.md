...
| D-055 | Hotfix 采用 Jira key 绑定的 develop 直合 main | `maintainer/scripts/hotfix.sh` 只保留 `publish --jira-id <KEY>`：不创建分支、PR、Tag，不调用 Jira/`gh`，不执行完整发布验证或额外人工门禁。显式命令调用即为授权；脚本校验干净且已同步的 `develop`，自动合并固定的 `origin/main` 与 `origin/develop`，生成带 Jira key 的 Merge commit，并用 atomic push 同步远端 `main/develop` 后回读；内容冲突时失败关闭。本例外不放宽正常发布、普通直推、强推或历史改写规则。 |
