...
2. `ao-work auth` 区分 Git commit 身份、Git SSH 远端授权和 GitHub CLI 账户，并要求明确选择 `global` 或 `installation`。`global` 只复用机器授权；`installation` 使用安装专属 SSH、隔离 `gh` 与 SSH-over-443，首次登录走交互式官方设备流程。
3. 写入前只读检查已有身份、SSH/`gh` 路径和 managed clone SSH 配置；受管差异要求绑定 `change_digest` 的精确确认，私钥、不同 `gh` 账户、自定义或非受管配置不覆盖。通过后写入 `user/identity.yaml` 和 `user/.env`，权限固定为 `0600`；token 不进入参数、日志或输出。
4. `workspace init` 从当前安装读取 `agent_id`、Jira 账户、Git 提交身份、Git/`gh` 授权模式和 GitHub login，不接收工作空间级身份或凭证参数。
5. Runtime 从 Project Profile 推导 Jira Connection、站点、Project Key、仓库和状态映射，并展示安装身份、授权模式、项目、源码池及脱敏账户供确认。
6. 确认后执行无副作用预检：工作空间边界、受管路径、已有配置、安装身份指纹、Profile/Connection、Jira 身份与 Project 访问、源码池和仓库访问。
7. 全部通过后准备源码资产，写入 Profile overlay、developer AI 入口、Skill 副本和可重建索引，最后写入 schema v5 `.agentic-ops/agent.json`。
8. `agent.json` 只保存项目事实与 `install_identity_ref`；不保存 `agent_id`、Jira accountId、Git 执行身份、授权模式或 token，不生成工作空间 `.agentic-ops/.env`。
9. Runtime 执行初始化后 preflight；通过后才允许读取或操作真实 Jira 任务。直接 Git/`gh` 操作使用选定授权环境，项目验证子进程不得继承安装 SSH/`gh` 凭证。

### 旧工作空间处理

- schema v4 及更早版本和工作空间 `.agentic-ops/.env` 不再是授权来源。
...
- 旧身份缺授权模式、已有授权差异未精确确认、非受管配置、不同安装 `gh` 账户、既有私钥或自定义 `core.sshCommand` 时失败关闭，不静默覆盖。
- `workspace init` 缺安装身份或凭证时返回 `install_identity_missing` 或 `jira_credentials_missing`，明确提示 `ao-work auth`。
- schema v4 及更早版本返回升级阻断，不回退读取工作空间 `.env`。
...
- `global` 模式不产生全局写入；`installation` 使用 SSH-over-443、隔离 SSH/`gh` 且不回退 Agent。提交身份、SSH 认证和 `gh` 账户不得混为同一证据。
- 已有授权受四态门禁保护，差异需要精确摘要确认；私钥、不同 `gh` 账户、自定义或非受管配置不能被普通命令覆盖。
- `workspace init` 不接收身份、email、Git/GitHub 或 token 参数。
- 新工作空间固定 schema v5，只保存 `install_identity_ref` 和项目配置，不创建 `.agentic-ops/.env`。
- schema v4 及更早版本在凭证读取和网络访问前失败关闭，不进行隐式迁移。
...
- 工作空间缺安装授权、身份指纹漂移或 schema v4 及更早版本时，必须在旧凭证读取和网络访问前停止。
...
- schema v5 初始化与 schema v4 及更早版本失败关闭测试。
