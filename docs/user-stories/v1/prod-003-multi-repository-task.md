...
- 接管进入 `task_intake` 前，当前 task/run 只准备一次 Jira 接管版本水印：该水印精确覆盖项目 Profile 配置的字符串字段，并且只有写后回读与当前 Product Root 版本一致时才能推进；外部结果不明、事务类型或版本变化均失败关闭。
...
- Jira 接管版本水印只有在当前 task/run 准备了字段和载荷摘要完全匹配的意图时才能自动放行一次；普通字段编辑、创建任务、夹带字段或重复写入均不得借用该许可。
...
- 接管版本水印、接管与 Tests Passed 状态意图、精确 Jira 门禁、缺字段跳过、写后回读，以及 PR Ready 三类事实核对结果。
