...
PROD-001 包含自动源码缓存的职责边界；具体源码准备与恢复以单任务工位合同为准。

PROD-003 覆盖初始 Jira 快照、本地确认与非阻断同步的恢复验收；同步状态和真实质量验收分别核对，具体用法链接质量指引。
...
| [PROD-003 推进并恢复多仓库研发任务](prod-003-multi-repository-task.md) | 完整独立工程、单 current 与多工位隔离、接管版本水印、质量检查点、非阻断 Jira 状态同步、PR Ready 核对、恢复清理及任务证据 |
