...
AgenticOps 问题修复与业务任务连续执行分轨推进。AO 故障本身不能直接终止业务开发；安全恢复失败且继续开发会与 AO 控制功能冲突时，必须进入 `agenticops_continuity_decision`，由人工在明确任务、仓库、分支、动作、验证和时效范围内决定是否改用项目原生流程。该决策不允许业务会话修改 AgenticOps 源头，不把降级执行写成 Runtime 成功，也不自动授权合并、发布、Tag、强推、历史改写或结果不明确的外部写入重试。
