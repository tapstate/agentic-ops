...
研发工程师确认版本化设计或修复计划时，可以一次性授予工作项级连续执行授权。有效授权覆盖当前任务范围内的实现、验证、提交、任务分支推送、必要 Jira 回写以及创建或更新拉取请求，AIAgent 不再逐项中断，完成后统一提交拉取请求审查包。合并、发布、Git Tag、强推、历史改写和范围变化不在该授权内。
