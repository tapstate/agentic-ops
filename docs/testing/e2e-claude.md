...
1. 执行 `<Product Root>/agenticops init --workspace <项目工作空间> --project tapdata --agent claude`。
2. 通过 `<Product Root>/agenticops start --agent claude --workspace <项目工作空间>`
   启动 Claude，要求接管两个 TapData 测试任务并保持两者 active。
