...
当前阶段优先维护 AgenticOps 的项目定位、规则、架构、用户故事、操作契约、AI 员工手册和实施计划。设计审阅确认前，不开始实现运行代码，除非用户明确要求进入实现。
...
第一阶段主 CLI 使用 Go 实现，统一入口为 `agent-task-ops`。shell 只用于 `curl | bash` 安装引导、轻量环境检测、下载或切换 Go release 二进制，不承载 Jira、GitHub、Git、Operation Contract、policy gate、evidence 或 feedback 的业务逻辑。

`~/.agentic-ops` 是全局安装和配置目录，不是具体项目运行目录。具体项目运行目录是项目 AI 工作空间，例如 `tapstate` 或 `tapdata`。

## 测试与验证

引入运行代码后，必须在同一变更中补充可执行验证命令。当前文档阶段至少使用这些检查：
