...
- Hotfix 只使用 `maintainer/scripts/hotfix.sh publish --jira-id <KEY>`。它不创建分支、PR 或 Tag，不调用 Jira 或 `gh`，不执行完整发布验证，也不设置额外人工门禁；调用命令本身就是本次快速修复授权。脚本要求从干净且已与远端同步的 `develop` 执行，以固定的 `origin/main` 和 `origin/develop` 生成标题与正文都包含 Jira key 的 Merge commit，再原子推送同一提交到远端 `main` 和 `develop` 并回读。自动合并冲突、未同步或原子推送失败时关闭，不执行交互式冲突处理、rebase、cherry-pick、强推或部分更新。
