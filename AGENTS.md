...
- 将用户的行动请求推进到可审查的结果；已明确的目标、输入和仍有效的授权跨轮次保留，不因 Skill 的“先确认”措辞重复询问。普通实现选择自行判断；缺少影响正确性、范围或权限的输入时，只询问缺失项，并继续不依赖回答的准备工作。
- 用户执行中补充要求时，保留已完成工作，检查范围、风险和验证是否变化；状态问题先简短回答，再继续原任务。取消或改目标时停止受影响操作，回读在途操作结果，不把迟到结果当成新目标的证据。
- 独立只读检查可使用平台原生并行工具；依赖前一步结果、同一工作树写入、阶段推进及外部副作用保持有序。只有用户或适用 Skill 明确要求子代理时才委派，交代范围、写入边界和证据要求，由主 Agent 核验结果。
- 验证覆盖变更与固定验收合同；没有新修改、失败或未解决风险时，不重复已通过检查。等待工具或人工答复时保留操作标识，使用平台可用的等待/事件能力，不以超时推定成功或授权。
- 中文沟通先说明结果或关键发现，再说明证据、限制和下一步；区分事实、推测与建议。需要人工决定时，给出具体对象、影响和要求确认的规则来源，不只报告“流程要求”。
...
- 提交标题使用第一个 `git commit -m "<title>"` 参数。
- 正文每个自然段必须使用独立的 `-m "<paragraph>"` 参数，Git 会自动以空行分隔各段。
- 禁止在任何 `-m` 参数中传入字面 `\n`、`\\n`、`echo -e`、`printf`，或依赖下游二次解析来形成多段正文。
- 正文确实需要复杂格式时，使用 `git commit -F <message-file>`；文件内容必须包含真实换行。
- 发起提交前，Agent 必须逐段检查提交参数中的标题、变更、原因、验证和风险；任何参数含字面 `\n` 时停止并改正。

推荐模板：

```sh
git commit \
  -m "fix(workspace): AO-108 中文主题" \
  -m "变更：……" \
  -m "原因：……" \
  -m "验证：……" \
  -m "风险：……"
