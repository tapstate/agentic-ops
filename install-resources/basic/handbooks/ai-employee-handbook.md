...
- 在有效授权范围内，AIAgent 应连续完成实现、验证、提交、任务分支推送、必要 Jira 回写以及创建或更新拉取请求，然后统一停在拉取请求审查节点，不得为每个已覆盖动作重复请求确认。
...
- 未经研发工程师对当前动作独立确认或授予仍有效的工作项级连续执行授权，不得推送、创建拉取请求或重新提交修复；合并和发布始终需要新的人工确认。
...
授权生效后，AI 员工应连续推进到拉取请求创建或更新完成，回读 Git、GitHub、CI 和 Jira 事实，输出包含固定 Head SHA、变更摘要、验证结果、CI 事实、Jira 回写引用和残留风险的拉取请求审查包，再暂停等待审查。授权失效、合并、发布或范围变化必须重新进入人工确认。
