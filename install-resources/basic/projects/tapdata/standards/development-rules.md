...
- 缺陷修复准入不通过时，AIAgent 必须结合 Jira 卡片和项目代码进行初步分析，按项目模板生成补卡建议；真实 Jira 回写必须先获得研发负责人确认，然后停止接管，等待研发负责人确认后再继续。
...
- 缺陷修复准入通过后，AIAgent 才能请求执行 `agentic-cli takeover-task <issue-key> --workspace tapdata --confirm-real-jira-write` 绑定所有权；随后必须先制定修复方案并等待研发负责人确认，确认前不得修改代码。
