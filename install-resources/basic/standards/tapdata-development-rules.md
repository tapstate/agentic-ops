...
- AIAgent 应优先使用 `agentic-cli switch-branch plan <branch_spec> --workspace tapdata` 生成分支对齐计划，确认无 blocked 行后才能执行 `agentic-cli switch-branch apply <branch_spec> --workspace tapdata`。
...
- `switch-branch apply` 允许按计划对脏仓库临时 `stash push -u`、切换分支后 `stash pop`；若 stash 或 pop 失败必须停止，不能继续跨仓切换。
