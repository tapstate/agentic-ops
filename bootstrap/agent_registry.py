...
def discover(product_root):
    ...
    for path in sorted(agent_root.glob("*/manifest.json")):
        ...
        required = {"schema_version", "name", "adapter_version", "artifacts", "launch"}
        optional = {"retired_artifacts", "skill_target"}
        if not isinstance(document, dict) or not required <= set(document) or set(document) - required - optional:
            raise ValueError("Agent Manifest v3 字段无效：%s" % path)
        ...
        skill_target = document.get("skill_target")
        if skill_target is not None:
            if not isinstance(skill_target, str) or not skill_target:
                raise ValueError("Agent Skill 目标无效：%s" % agent_id)
            target_path = Path(skill_target)
            if target_path.is_absolute() or any(
                part in ("", ".", "..") for part in target_path.parts
            ):
                raise ValueError("Agent Skill 目标必须是受控相对路径：%s" % agent_id)
        if "project_skill_target" in document:
            raise ValueError(
                "Agent Manifest 不再支持 project_skill_target，请改用 skill_target：%s"
                % agent_id
            )
