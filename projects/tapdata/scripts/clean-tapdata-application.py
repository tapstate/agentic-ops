#!/usr/bin/env python3
"""仅清理 tapdata-application 仓库明确生成的 target/Python 缓存，不处理数据库或容器。"""
import argparse
...
if root.name != 'tapdata-application' or not (root / '.git').is_dir():
    raise SystemExit('必须显式指定 tapdata-application 独立仓库')
