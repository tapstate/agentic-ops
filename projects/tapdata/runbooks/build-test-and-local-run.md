...
| 内容 | 要求 |
|---|---|
| 测试级别与安排 | 经 tapdata-wiki 查询总体设计、层级判定指南及编写排错经验，结合实际被测边界判断级别并解释依据；记录适用文章路径、环境、执行入口、本地与 CI 验证安排，写入同一 Q2 方案的 integration_tests，不复制 Wiki 定义 |
...
框架不支持时，Agent 说明缺少的能力、受影响场景和可用替代验证；确认框架不支持后，返回主流程先检索已有事项，再询问研发是否创建/关联 Jira 后续任务。临时查询失败、未提供本地环境等先说明恢复方式，不默认转成业务任务。创建与本次验收缺口的处置分别处理，等待答复不影响无依赖工作；不新增门禁，也不绕过已有应测范围和失败处置。用户接受的缺口仍是未覆盖，不登记为 PASS；验收预期不清、事实不可信时暂停相应判断，不凭实现反推预期。
...
2. 目标分支 workflow 的触发、路径过滤、矩阵、profile 和测试命令，与本任务应测模块及场景对照。测试报告列出实测用例、成功/失败/错误/跳过计数及缺失模块；没有目标用例或零用例均不能证明通过。即使其它 Checks 全部成功，也须将集成测试的具体未覆盖范围交研发明确处置。
...
对照质量检查项导入实际 `execute` 证据：`origin=ci`，实际用例与代码版本，`source_ref` 指向精确运行/报告。未执行使用 NOT_RUN、跳过使用 SKIPPED；缺报告、零用例或版本不明记录为 UNKNOWN，并在 observation 保留具体原因；不能填 PASS。完全未触发运行时按[共同验证材料](../../../docs/usage/quality-checkpoints.md#共同验证材料)使用 not_triggered，不能虚构 run/attempt；将每项方案 scope 以 method=integration 对应到 verification.results，缺口保留原结果并请求研发确认具体原因和处置。研发确认不等于 PASS，也不解除原验收、PR Checks 和服务端条件。报告总结以 Jira 讨论回写并回读，至少包含各仓代码/测试版本、应测/实测/未覆盖范围、运行和报告链接、结果计数、失败归因与后续处理。评论失败保留同步待办，不丢失本地分析。
