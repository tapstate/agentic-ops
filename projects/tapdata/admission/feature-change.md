...
|---|---|---|---|---|
| 验收标准 | `acceptance_criteria` | Jira 描述「验收标准」章节 | 关键用户场景完成验证，新增行为有自动化测试覆盖 | 需求完成后怎样判断可验收 |
| 目标仓库 | `target_repo` | Jira 组件/标签/项目仓库映射 | tapdata/tapdata | 本次应修改或检查的仓库；必须能在 profile.json 中查到分支映射 |
| 验证方式 | `verification_method` | Jira 验证方式字段（customfield_10049） | 执行对应模块单元测试和必要手工验证 | 完成后必须执行的验证 |
